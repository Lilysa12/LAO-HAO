...
const QrMeja = () => {
  ...
  const getIconClass = (path) =>
    location.pathname === path ? "menu-icon-svg" : "menu-icon-svg icon-white";
  ...
  const filteredTables = tables.filter((table) => {
    ...
    if (
      activeTab === "Nonaktif" &&
      (table.status === "pending" || table.status === "lunas")
    )
      return true;
    return false;
  });

  // FUNGSI MENCETAK QR CODE KE PDF (DINAMIS MENGGUNAKAN DOMAIN AKTIF)
  const handlePrintQR = (tableName) => {
    // Ambil domain otomatis saat ini (localhost atau domain prod)
    const baseUrl = window.location.origin;

    // Format baru dengan parameter query string
    const urlOrder = `${baseUrl}/order?cabang=Malang&meja=${tableName}`;
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .footer { margin-top: 15px; font-size: 12px; color: #94a3b8; font-family: monospace; word-break: break-all; }
          </style>
        </head>
        <body>
          <div class="card">
  ...
  const handleGenerateNewQR = () => {
    if (!inputMeja)
      return alert("Silakan masukkan nomor meja terlebih dahulu!");
      ...
      <main className="main-content">
        <header className="topbar">
          <div className="breadcrumb">
            ...
            <span className="text-black font-bold">Qr Meja</span>
          </div>
        ...
        <div className="content-wrapper">
          ...
          >
            <div>
              ...
              <p className="page-subtitle">
                Kelola meja dan cetak QR Code untuk self-ordering
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            >
              Menarik data meja dari database...
            </div>
          ) : (
            <div className="qr-grid">
              {filteredTables.map((table) => {
                ...
                // Ambil domain origin dinamis untuk setiap baris kartu di grid
                const currentBaseUrl = window.location.origin;
                const dynamicUrlOrder = `${currentBaseUrl}/order?cabang=Malang&meja=${table.table_number}`;

                return (
                  <div
                    key={table.id}
                    className={`qr-card ${isNonaktif ? "nonaktif" : ""}`}
                    style={{
                  ...
                  >
                    ...
                    >
                      <span
                        style={{
                          color: isNonaktif ? "#ef4444" : "#10b981",
                          backgroundColor: isNonaktif ? "#fee2e2" : "#d1fae5",
                          padding: "4px 8px",
                          borderRadius: "4px",
                        }}
                      >
                    ...
                    <div
                      ...
                      onClick={() =>
                        !isNonaktif && handlePrintQR(table.table_number)
                      }
                    >
                      {/* GENERATE GAMBAR DENGAN URL QUERY STRING DINAMIS */}
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(dynamicUrlOrder)}`}
                        alt="QR"
                        style={{
                          width: "100%",
                          height: "100%",
      ...
      {isModalOpen && (
        <div className="modal-overlay">
          ...
          >
            ...
            >
              QR Code ini dapat dicetak untuk meja tambahan atau event khusus.
            </p>
