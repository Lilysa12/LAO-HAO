        ...
        <div className="pos-layout">
          <div className="pos-menu-section">
            ...
            <div className="search-bar-wrapper">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="search-icon-svg">
                <circle cx="11" cy="11" r="8"></circle>
                <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
              </svg>
              <input type="text" placeholder="Cari menu (ex: Nasi Goreng)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pos-search-input" />
            </div>
            ...
            <div className="menu-grid">
              {isLoading ? (
                <p style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '20px' }}>Memuat menu...</p>
          ...
          <aside className="pos-cart-section">
            ...
            <div className="customer-info-inputs">
              <div className="input-group-main">
                ...
                <input type="text" placeholder="Ex: Budi" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
              </div>
              <div className="input-group-table">
                <label>NO. MEJA</label>
                <input type="text" placeholder="Ex: 12" value={tableNumber} onChange={(e) => setTableNumber(e.target.value)} disabled={orderType === 'Takeaway'} />
              </div>
            </div>
                  ...
                  <div key={item.id} className="cart-item">
                    <div className="cart-item-top">
                      ...
                      <div className="qty-controls">
                        ...
                        <button onClick={() => updateQty(item.id, 1)} className="qty-btn-plus">+</button>
                      </div>
                    </div>
                    <span className="cart-item-price">{formatRupiah(item.price * item.qty)}</span>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="cart-summary-box">
              ...
              <div className="summary-row"><span>PB1 (10%)</span><span>{formatRupiah(taxPB1)}</span></div>
              <div className="summary-row"><span>Service (5%)</span><span>{formatRupiah(serviceCharge)}</span></div>
              ...
              <div className="cart-actions">
                <button onClick={() => handleCheckout('BELUM BAYAR')} disabled={isProcessing} className="btn-open-table">
                  Open Table (Bayar Nanti)
                </button>
                <button onClick={() => handleCheckout('LUNAS')} disabled={isProcessing} className="btn-checkout">
